...
public class Frustum : MonoBehaviour
{
    ...
    // This function creates a frustum from a mirror, given the position of
    ...
    public static Frustum CreateFrustumFromMirror(FlexibleMirror mirror, Camera c, GameObject container, Material material, Color color)
    {
        // An array of the 8 vertices which make up the frustum
        Vector3[] fv = new Vector3[8];

        // Get the minimum and maximum bounds
        Vector3 minBound = mirror.minBound;
        Vector3 maxBound = mirror.maxBound;

        // Calculate the four vertices which make up the near plane -
        // These will just be the min and max bounds of our mirror
        fv[0] = mirror.transform.TransformPoint(new Vector3(minBound.x, minBound.y, maxBound.z));
        fv[1] = mirror.transform.TransformPoint(new Vector3(maxBound.x, minBound.y, maxBound.z));
        fv[2] = mirror.transform.TransformPoint(new Vector3(maxBound.x, minBound.y, minBound.z));
        fv[3] = mirror.transform.TransformPoint(new Vector3(minBound.x, minBound.y, minBound.z));
        ...
        // length of the camera frustum.
        // More simplified, we get the direction of the vector from the camera
        // to the near plane vertex, make it's length the distance from the camera 
        // near plane to the camera far plane

        // This is a bit different from our portal, as our cameras will move
        // further away from each other in the event that we have a hall of mirrors
        // but we still want to keep reflecting, even if the camera's far
        // plane is not far enough
        fv[4] = fv[0] + ((fv[0] - c.transform.position).normalized * (c.farClipPlane - c.nearClipPlane));
        fv[5] = fv[1] + ((fv[1] - c.transform.position).normalized * (c.farClipPlane - c.nearClipPlane));
        fv[6] = fv[2] + ((fv[2] - c.transform.position).normalized * (c.farClipPlane - c.nearClipPlane));
        fv[7] = fv[3] + ((fv[3] - c.transform.position).normalized * (c.farClipPlane - c.nearClipPlane));
    ...
    // This function will check to see if the frustum contains a mirror.
    // We cheat a little bit here - mirrors are flat rectangles in this code,
    // So rather than creating a bounding sphere, etc, we just sample a bunch
    // of points in a grid within the boundary of the mirror, and if any of them
    // are inside the mirror, we assume the mirror is in the frustum.
    // It's not a perfect solution, but it's a simple one which works 99% of the
    // time
    public bool containsMirror(FlexibleMirror mirror)
    {
        // Get the minimum and maximum bounds
        Vector3 minBound = mirror.minBound;
        Vector3 maxBound = mirror.maxBound;

        // If the mirror is facing the opposite direction of the frustum, we
        // consider it to not be in the frustum. This means that mirrors are one
        // directional.
        if (Vector3.Dot(transformedFrustumNormals[1], -mirror.transform.up) < -0.5)
            return false;

        // Rather than properly determining if a mirror is in the frustum, lets just sample some points
        // which are on the mirror plane and see if any of them are in the frustum
        ...
        for (float z=0; z<=1; z+=.1f)
        {
            for (float x=0; x<=1; x+=.1f)
            {
                ...
                // right corners at x by z intervals
                Vector3 testPoint = new Vector3(Mathf.Lerp(minBound.x, maxBound.x, x), minBound.y, Mathf.Lerp(minBound.z, maxBound.z, z));

                // If any point is inside the frustum, we immediately say the
                // mirror is inside the frustum
                if (containsPoint(mirror.transform.TransformPoint(testPoint), 0))
                    return true;
            }
        }

        // If no points were inside the frustum, we say the mirror is not
        // inside the frustum
        return false;
    }
    ...
    public void UpdateFrustum(Transform frustumContainerTransform = null)
    {
        if (frustumContainerTransform == null)
            frustumContainerTransform = transform;

        // Loop through all 6 of the frustum planes
        for (int i = 0; i < 6; i++)
        {
            // And update the transformed frustum centers and transformed frustum normals
            transformedFrustumCenters[i] = frustumContainerTransform.TransformPoint(frustumCenters[i]);
            transformedFrustumNormals[i] = frustumContainerTransform.TransformDirection(frustumNormals[i]);
        }
